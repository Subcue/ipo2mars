...
export function Moon({ orbitRadius = 3.0, radius = 0.18 }: { orbitRadius?: number; radius?: number }) {
  ...
  useFrame((_, dt) => {
    ...
    pivot.current.rotation.y += dt * 0.028 // slow orbit (~3.7 min per revolution)
    if (body.current) body.current.rotation.y += dt * 0.028 // tidal lock
  })
  ...
  return (
    ...
    <group ref={pivot} rotation={[0.09, 2.2, 0]}>
      <mesh ref={body} position={[orbitRadius, 0.1, 0]}>
